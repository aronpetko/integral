...
-   Made handling of `#` consistent with `fmt::format`.
...
-   Removed deprecated implicit conversions for enums and conversions to
    primitive types for compatibility with `fmt::format` and to prevent
...
-   Formatting floating-point numbers no longer produces trailing zeros
    by default for consistency with `fmt::format`. For example:
...
-   The `'L'` specifier for locale-specific numeric formatting can now
    be combined with presentation specifiers as in `fmt::format`. For
    example:
...
-   Changed the default floating point format to not include `.0` for
    consistency with `fmt::format` and `std::to_chars`
...
-   The `'='` format specifier is now disabled by default for
    compatibility with `fmt::format`. It can be enabled via the
...
-   Removed the following deprecated APIs:
    ...
    -   `FMT_DEPRECATED_PERCENT` - incompatible with `fmt::format`
...
-   Implemented the `'L'` specifier for locale-specific numeric
    formatting to improve compatibility with `fmt::format`. The `'n'`
...
-   {fmt} no longer converts `float` arguments to `double`. In
    ...
    floats and makes `fmt::format` consistent with `fmt::format` specs
...
-   Added `basic_format_parse_context` for consistency with C++20
    `fmt::format` and deprecated `basic_parse_context`.
...
-   Separated formatting and parsing contexts for consistency with
    [C++20 fmt::format](http://eel.is/c++draft/format), removing the
