  ...
  for (int depth = 1; depth <= time_mgmt_.GetSearchDepth(); depth++) {
    for (thread.pv_move_idx = 0; thread.pv_move_idx < multi_pv;
      ...
      if (depth >= kAspWindowDepth) {
        alpha = std::max(-kInfiniteScore, cur_best_move.avg_score - window);
        beta = std::min(kInfiniteScore, cur_best_move.avg_score + window);
      }
  ...
  while (const auto move = move_picker.Next()) {
    ...
    if (in_root) {
      ...
      if (const auto root_move = thread.root_moves.FindRootMove(move)) {
        root_move->avg_score = root_move->avg_score == kScoreNone
                                 ? score
                                 : (root_move->avg_score + score) / 2;
