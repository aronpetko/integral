...
#ifndef DATAGEN
  // Adjust based on the number of material left on the board
...
Score Searcher::PVSearch(Thread &thread,
  ...
  if (!stack->excluded_tt_move && !in_pv_node && can_use_tt_eval &&
    ...
    // If we failed high here and the previous move was a capture move, then we
    // give a malus to the previous move since it will fail low
    if (tt_entry->score >= beta && (stack - 1)->move &&
        (stack - 1)->moves_seen && !(stack - 1)->capture_move) {
