...
Score Search::PVSearch(Thread &thread,
  ...
  if (past_stack && !in_check) {
    ...
    // previous turns
    const auto diff = stack->static_eval - past_stack->static_eval;
    const auto scaled_diff =
        (diff * 1000) / static_cast<int>(improving_rate_divisor);
    stack->improving_rate =
        std::clamp(past_stack->improving_rate + scaled_diff, -1000, 1000);
  }
  ...
  if (!in_pv_node && !in_check && stack->eval < kTBWinInMaxPlyScore) {
    ...
    if (depth <= rev_fut_depth && !stack->excluded_tt_move &&
      ...
      const int futility_margin =
          (depth * 1000 - 3 * stack->improving_rate / 2) / 1000 *
              rev_fut_margin +
