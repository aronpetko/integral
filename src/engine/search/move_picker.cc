    ...
    switch (piece_type) {
      case kPawn:
        if (move_gen::PawnAttacks(to, us) & enemy_king) {
          ...
          move_score +=
              1000 * move_gen::PawnAttacks(state.Pawns(us), us).IsSet(to);
        }
        break;
      case kKnight:
        if (move_gen::KnightMoves(to) & enemy_king) move_score += 2000;
        break;
      case kBishop:
      ...
      case kRook:
        ...
        break;
      case kQueen:
        if (move_gen::QueenMoves(to, occupied) & enemy_king) move_score += 4500;
        break;
    }
  }
