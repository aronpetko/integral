#ifndef INTEGRAL_NON_PAWN_HISTORY_H
#define INTEGRAL_NON_PAWN_HISTORY_H

namespace search::history {

TUNABLE(kNonPawnHistFill, -955, -3000, 0, false);

class NonPawnHistory {
 public:
  NonPawnHistory() : table_({}) {
    table_.fill(kNonPawnHistFill);
  }
 ...
 private:
  [[nodiscard]] int GetIndex(const BoardState &state, Color color) const {
    return state.non_pawn_keys[color] & 16383;
  }

 private:
  MultiArray<I16, 16384, kNumPieceTypes, kSquareCount> table_;
};

}  // namespace search::history

#endif  // INTEGRAL_NON_PAWN_HISTORY_H