#ifndef INTEGRAL_PAWN_CAPTURE_HISTORY_H
#define INTEGRAL_PAWN_CAPTURE_HISTORY_H
...
class PawnCaptureHistory {
 public:
  PawnCaptureHistory() : table_({}) {}
  ...
  void UpdateScore(const BoardState &state, Move move, I16 depth) {
    ...
    I16 &score = table_[GetIndex(state)][state.turn][state.GetPieceType(from)]
                       [to][captured];
  ...
  void Penalize(const BoardState &state, I16 depth, MoveList &captures) {
    ...
    for (I16 i = 0; i < captures.Size(); i++) {
      ...
      I16 &bad_capture_score = table_[GetIndex(state)][state.turn]
                                     [state.GetPieceType(from)][to][captured];
  ...
  [[nodiscard]] I16 GetScore(const BoardState &state, Move move) const {
    ...
    const auto captured =
        move.IsEnPassant(state) ? kPawn : state.GetPieceType(to);
    return table_[GetIndex(state)][state.turn][state.GetPieceType(from)][to]
                 [captured];
  }
...
#endif  // INTEGRAL_PAWN_CAPTURE_HISTORY_H