...
Tunable lmr_quiet_div("lmr_quiet_div", 2.02, 1.5, 3.0, 0.1);
Tunable lmr_tact_base("lmr_tact_base", -0.23, -0.5, 0.2, 0.05);
...
Tunable asp_window_growth("asp_window_growth", 1.58, 0.1, 1.0, 0.05);
...
Tunable null_move_re("null_move_re", 203, 100, 400, 20);

Tunable iir_depth("iir_depth", 4, 2, 8, 1);

Tunable improving_rate_divisor("improving_rate_divisor", 31.0, 5.0, 100.0, 15.0);

Tunable lmp_base("lmp_base", 5.0, 1.5, 5.0, 0.5);
Tunable lmp_mult("lmp_mult", 2.51, 1.0, 4.0, 0.25);

Tunable fut_prune_depth("fut_prune_depth", 8, 6, 12, 1, true);
Tunable fut_margin_base("fut_margin_base", 190, 100, 250, 20);
Tunable fut_margin_mult("fut_margin_mult", 96, 50, 200, 15);

Tunable see_prune_depth("see_prune_depth", 8, 6, 12, 1, true);
Tunable see_quiet_thresh("see_quiet_thresh", -65, -100, -20, 6);
Tunable see_noisy_thresh("see_noisy_thresh", -120, -200, -50, 8);

Tunable hist_prune_depth("hist_prune_depth", 5, 3, 8, 1, true);
Tunable hist_thresh_base("hist_thresh_base", -521, -1000, 500, 100);
Tunable hist_thresh_mult("hist_thresh_mult", -1742, -3000, -250, 300);

Tunable lmr_hist_div("lmr_hist_div", 11525, 5000, 20000, 750);
