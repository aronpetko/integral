...
void Initialize(Board &board, search::Search &search) {  // clang-format off
  ...
  listener.RegisterCommand("checks", CommandType::kUnordered, {
    CreateArgument("move", ArgumentType::kOptional, LimitedInputProcessor<1>()),
  }, [&](Command *cmd) {
    const auto move = cmd->ParseArgument<std::string>("move");
    if (move) fmt::println("{}", board.MoveGivesCheck(Move::FromStr(*move, board.GetState())));
  });
