...
constexpr SquareTable<BitBoard> GenerateAdjacentFiles() {
  ...
  for (Square square = 0; square < Squares::kSquareCount; square++) {
    adjacent_files[square] = kFileMasks[File(square)];
    adjacent_files[square] |= Shift<Direction::kWest>(adjacent_files[square]) |
                              Shift<Direction::kEast>(adjacent_files[square]);
  }
